...
const unauthorizedStt = await json('/api/transcribe', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ audioBase64: 'AAAA', format: 'webm' }) });
...
const anonymousSnapshot = await json(`/api/room?code=${encodeURIComponent(created.code)}`);
const lines = snapshot.payload.room?.utterances || [];
const checks = {
  demoSessionIssued: session.response.ok && typeof session.payload.token === 'string',
  paidEndpointProtected: unauthorizedStt.response.status === 401,
  ...
  privateSnapshot: anonymousSnapshot.response.status === 401,
